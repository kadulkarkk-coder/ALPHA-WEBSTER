...
def _print_voice_diagnostics(launcher):
    ...
    i = d['input']
    print(f"║ Microphone       : {'OK' if i['available'] else 'FAIL'} ({i['backend']})")
    print(f"║ Device           : {i['device']}")
    print(f"║ Listening        : {i['listening']}")
    print(f"║ RMS / threshold  : {i['rms']} / {i['threshold']}")
    print(f"║ STT latency      : {d['stt']['latency_ms']} ms")
    w = d['wake_word']
    print(f"║ Wake word        : {w['word']} | detected={w['detected']} | score={w['score']}")
    c = d['conversation']
    print(f"║ Conversation     : active={c['active']} | turns={c['turns']}")
    b = d['barge_in']
    print(f"║ Barge-in         : enabled={b['enabled']} | ready={b['ready']} | interrupted={b['last_interrupted']}")
    o = d['output']
    print(f"║ TTS              : {'OK' if o['available'] else 'FAIL'} ({o['backend']}) | {o['tts_latency_ms']} ms")
...
def _print_vision_status(launcher):
    health = launcher.vision_manager.health()
    print("\nVision status:")
    print(f"  enabled:          {health['enabled']}")
    print(f"  screen capture:   {'READY' if health['screen_capture'] else 'UNAVAILABLE'}")
    print(f"  camera capture:   {'READY' if health['camera_capture'] else 'UNAVAILABLE'}")
    print(f"  vision provider:  {'READY' if health['vision_provider'] else 'OCR/FALLBACK'}")
    print(f"  OCR provider:     {'READY' if health['ocr_provider'] else 'NOT CONFIGURED'}")
    print()
...
def run_cli(launcher):
    ...
    while True:
        ...
        if lower == "capabilities":
            ...
            print()
            continue
        if lower in {"vision", "vision status", "vision diagnostics"}:
            _print_vision_status(launcher)
            continue
        if lower == "voice":
            _print_voice_status(launcher)
            continue
        if lower == "voice diagnostics":
            _print_voice_diagnostics(launcher)
            continue
