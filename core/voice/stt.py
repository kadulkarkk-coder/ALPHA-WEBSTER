...
class FasterWhisperSpeechBackend(SpeechToTextBackend):
    ...
    The microphone stream is opened once and remains alive for the whole voice
    session. Each utterance is extracted from the shared audio queue. This is
    deliberate: repeatedly creating/destroying Windows InputStreams after TTS
    is unreliable on some laptop audio drivers and was causing Webster to stop
    accepting input after the first response.
    """
    ...
    def initialize(self):
        ...
        try:
            ...
            try:
                ...
                self.block_size = max(160, int(self.sample_rate * .03))
    ...
    def start(self):
        """Open the microphone stream once for the entire voice session."""
        ...
        try:
            ...
            self._stream = None
            self._error = f"Microphone stream failed to start: {error}"
    ...
    def listen(self, timeout, phrase_timeout):
        ...
        if self._stream is None:
            return None
        # Discard everything captured during Webster's response. This prevents
        # TTS echo from becoming the next user utterance.
        self._clear_audio()
        self._stop_event.clear()
        captured = []
        pre_roll = deque(maxlen=max(1, int(self.pre_roll_seconds / .03)))
        speech_started = False
        voiced_blocks = 0
        start_time = time.monotonic()
        last_voice = start_time
        block_seconds = self.block_size / self.sample_rate
        silence_limit = max(.8, self.silence_duration)
        phrase_limit = max(2., min(float(phrase_timeout), self.max_phrase_seconds))
        timeout_limit = max(.5, float(timeout))
        calibration = []
        calibrated = False
        try:
            ...
            if 0 < peak < .15:
                audio = np.clip(audio * (.15 / peak), -1., 1.)
    ...
    def set_speaking(self, speaking, allow_barge_in=False):
