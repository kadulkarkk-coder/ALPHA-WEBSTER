...
class FasterWhisperSpeechBackend(SpeechToTextBackend):
    """Capture one complete natural utterance with sounddevice and transcribe it locally."""
    name="sounddevice_whisper"
    def __init__(self,config=None):
        self.config=config; self.sample_rate=int(getattr(config,"sample_rate",16000)); self.channels=1; self.block_size=max(160,int(self.sample_rate*int(getattr(config,"block_ms",30))/1000)); self.max_phrase_seconds=float(getattr(config,"max_phrase_seconds",20.0)); self.energy_threshold=float(getattr(config,"vad_energy_threshold",0.008)); self.vad_multiplier=float(getattr(config,"vad_multiplier",2.2)); self.silence_duration=float(getattr(config,"silence_duration",1.35)); self.pre_roll_seconds=float(getattr(config,"pre_roll_seconds",0.35)); self.start_blocks=max(1,int(getattr(config,"start_blocks",2))); self.model_name=str(getattr(config,"whisper_model","tiny.en")); self.device=str(getattr(config,"whisper_device","cpu")); self.compute_type=str(getattr(config,"whisper_compute_type","int8")); self.language=str(getattr(config,"language","en")).split("-")[0]; self.input_device=getattr(config,"input_device",None); self._model=None; self._available=False; self._listening=False; self._speaking=False; self._allow_barge_in=False; self._error=None; self._stop_event=Event(); self._lock=Lock(); self._queue=deque(); self._last_rms=0.0; self._last_threshold=self.energy_threshold; self._device_name=None
    def initialize(self):
        if self._available:return
        try:
            import sounddevice as sd
            device=self.input_device if self.input_device is not None else sd.default.device[0]; info=sd.query_devices(device,"input")
        ...
        except Exception as error: self._available=False; self._model=None; self._error=f"Voice input initialization failed: {error}"
    def listen(self,timeout,phrase_timeout):
        ...
        try:
            with sd.InputStream(device=self.input_device,samplerate=self.sample_rate,channels=1,dtype="float32",blocksize=self.block_size,callback=callback):
                ...
                while not self._stop_event.is_set():
                    now=time.monotonic()
                    with self._lock:block=self._queue.popleft() if self._queue else None
                    if block is None:
        ...
        except Exception as error:self._error=f"Microphone capture failed: {error}";return None
    ...
    def _transcribe(self,audio):
        ...
        except Exception as error:self._error=f"Whisper transcription failed: {error}";return None
    def set_speaking(self,speaking,allow_barge_in=False):self._speaking=speaking;self._allow_barge_in=allow_barge_in
    def stop(self):self._stop_event.set();self._listening=False
